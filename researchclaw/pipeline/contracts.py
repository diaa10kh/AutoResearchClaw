"""Stage I/O contracts for the 15-stage geotechnical journal-paper pipeline.
...
CONTRACTS: dict[Stage, StageContract] = {
    ...
    Stage.PROBLEM_DECOMPOSE: StageContract(
        ...
        dod=">=3 prioritized sub-questions identified for the geotechnical paper",
    ...
    Stage.SEARCH_STRATEGY: StageContract(
        ...
        dod=">=2 search strategies defined with verified geotechnical data sources",
    ...
    Stage.SYNTHESIS: StageContract(
        ...
        dod="Topic clusters + >=2 research gaps identified in geotechnical literature",
        error_code="E07_SYNTHESIS_WEAK",
    ),
    Stage.CONTRIBUTION_FRAMING: StageContract(
        stage=Stage.CONTRIBUTION_FRAMING,
        input_files=("synthesis.md",),
        output_files=("contribution_framing.md",),
        dod="Clear paper contribution statement with novelty, scope, and positioning relative to existing work",
        error_code="E08_FRAMING_FAIL",
    ),
    # Phase D: Paper Writing
    Stage.PAPER_OUTLINE: StageContract(
        stage=Stage.PAPER_OUTLINE,
        input_files=("synthesis.md", "contribution_framing.md"),
        output_files=("outline.md",),
        dod="Complete paper outline with section-level detail suitable for Q1 journal",
        error_code="E09_OUTLINE_FAIL",
    ),
    Stage.PAPER_DRAFT: StageContract(
        stage=Stage.PAPER_DRAFT,
        input_files=("outline.md",),
        output_files=("paper_draft.md",),
        dod="Full paper draft with all sections written for geotechnical numerical simulation journal",
        error_code="E10_DRAFT_FAIL",
    ),
    Stage.PEER_REVIEW: StageContract(
        stage=Stage.PEER_REVIEW,
        input_files=("paper_draft.md",),
        output_files=("reviews.md",),
        dod=">=2 simulated review perspectives with actionable feedback for geotechnical journal",
        error_code="E11_REVIEW_FAIL",
    ),
    ...
    Stage.QUALITY_GATE: StageContract(
        ...
        dod="Quality score meets Q1 journal threshold and approved",
        error_code="E13_GATE_REJECT",
        max_retries=0,
    ),
    Stage.EXPORT_PUBLISH: StageContract(
        ...
        dod="Final paper exported in target format for journal submission",
        error_code="E14_EXPORT_FAIL",
    ),
