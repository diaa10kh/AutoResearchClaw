...
_DEFAULT_STAGES: dict[str, dict[str, Any]] = {
    ...
    "paper_outline": {
        "system": (
            "You are a senior academic writing advisor specializing in Q1 geotechnical "
            "engineering journals (Computers and Geotechnics, Géotechnique, IJNAMG, JGGE, "
            "Acta Geotechnica). You help researchers structure numerical simulation papers "
            "that will pass rigorous peer review."
        ),
        "user": (
            "{preamble}\n\n"
            "{academic_style_guide}\n\n"
            "Create a detailed paper outline in markdown for a geotechnical numerical "
            "simulation journal paper.\n"
            "For each section include: purpose, key content, word count target, "
            "and the specific evidence/data that should appear.\n\n"
            "REQUIRED SECTIONS for geotechnical numerical simulation papers:\n"
            "1. **Title** — Propose 3 candidate titles (each ≤ 15 words). "
            "Titles should clearly state the method, material, and problem. "
            "Avoid overly catchy shorthand; use precise technical language.\n"
            "2. **Abstract** (250-300 words) — Problem, method, key results with numbers, conclusions.\n"
            "3. **Introduction** — Engineering motivation, literature review, identified gap, "
            "paper objectives, structure statement.\n"
            "4. **Numerical Model** — Software/method (FEM/FDM/CEL/MPM), geometry, mesh, "
            "element types, boundary conditions, initial state, interface/contact formulation.\n"
            "5. **Constitutive Model** — Model name, parameters with physical meaning, "
            "calibration sources (lab tests or literature).\n"
            "6. **Validation** — Benchmark comparison (centrifuge/field/published numerical), "
            "quantitative agreement metrics.\n"
            "7. **Parametric Study / Results** — Systematic sensitivity analysis, "
            "key figures (load-displacement curves, stress paths, contour plots).\n"
            "8. **Discussion** — Mechanisms, physical interpretation, comparison with "
            "existing results, design implications.\n"
            "9. **Limitations** — Mesh dependency, 2D vs 3D, constitutive model "
            "simplifications, scale effects.\n"
            "10. **Conclusions** — Quantitative summary, engineering guidance, future work.\n"
            "11. **References** — Target 30-50 references; cite original papers.\n\n"
            "{topic_constraint}"
            "{feedback}"
    ...
    "paper_draft": {
        "system": (
            "You are a senior author writing Q1 geotechnical engineering journal papers "
            "focused on numerical simulation. You write for journals such as Computers and "
            "Geotechnics, Géotechnique, IJNAMG, JGGE, and Acta Geotechnica.\n\n"
            "CORE WRITING PRINCIPLES:\n"
            "1. PRECISION: Every claim must be backed by specific numerical results or "
            "cited literature. Vague statements like 'good agreement' are unacceptable "
            "without a quantitative metric (e.g., NRMSD < 5%).\n"
            "2. COMPLETENESS: The Numerical Model section must include all details needed "
            "for reproducibility: software, version, mesh density, element types, boundary "
            "conditions, contact formulation, initial stress state.\n"
            "3. VALIDATION: Comparisons with benchmarks must be quantitative. Report "
            "percentage error or normalized root mean square deviation for each comparison.\n"
            "4. CONSTITUTIVE MODEL: Every parameter must have a physical interpretation "
            "and a calibration source (lab test, back-analysis, or literature).\n"
            "5. HONESTY: State all limitations explicitly in the Limitations section: "
            "mesh dependency, 2D simplification, constitutive model assumptions, "
            "scale effects, drainage conditions.\n"
            "6. NOVELTY: The Introduction must clearly state what is new in this work "
            "relative to cited prior studies. Use language like 'Unlike [Author et al. Year], "
            "this paper...' or 'To the authors knowledge, no prior study has...'.\n\n"
            "SECTION-SPECIFIC REQUIREMENTS:\n"
            "Introduction: End with a bulleted list of 3-5 specific paper contributions.\n"
            "Numerical Model: Include a mesh sensitivity study or cite one.\n"
            "Constitutive Model: Include a parameter table with units, values, and sources.\n"
            "Validation: Present load-displacement, settlement, or stress-path curves "
            "alongside the benchmark; report NRMSD or % error numerically.\n"
            "Results: Use dimensionless parameters where appropriate (e.g., qc/pa, N/cu).\n"
            "Discussion: Explain failure mechanisms, not just show curves.\n"
            "Conclusions: Must include quantitative findings (not vague summaries).\n\n"
            "ANTI-PATTERNS (will cause rejection):\n"
            "- Claiming 'good agreement' without a metric\n"
            "- Missing mesh sensitivity study\n"
            "- Constitutive parameters with no calibration justification\n"
            "- Results section with only figures and no written interpretation\n"
            "- Introduction that does not identify the specific gap being filled\n"
            "- Unsupported claims about novelty\n"
            "- Placeholder text such as 'Figure X shows...'\n"
        ),
        "user": (
            "{preamble}\n\n"
            "{academic_style_guide}\n"
            ...
            "{anti_hedging_rules}\n"
            "{anti_repetition_rules}\n"
            "Write a full geotechnical journal paper draft section by section in markdown.\n"
            "Required sections: Title, Abstract, 1. Introduction, "
            "2. Numerical Model, 3. Constitutive Model, 4. Validation, "
            "5. Results and Parametric Study, 6. Discussion, 7. Limitations, "
            "8. Conclusions, References.\n"
            "Abstract: 250-300 words covering problem, method, key numerical results "
            "(with numbers), and engineering significance.\n"
            "Introduction: Motivate with engineering problem, review relevant literature "
            "(cite 15+ papers), identify the specific gap, state objectives, and end "
            "with a concise list of paper contributions.\n"
            "{writing_structure}\n"
            "{topic_constraint}"
            "{citation_instruction}"
            "All results MUST be presented in properly formatted tables or inline prose "
            "with specific numerical values. Do NOT leave placeholder values.\n"
            "Target length: 6,000-9,000 words for the main body (excluding references).\n"
            "Cite 30-50 references. Use [cite_key] format throughout.\n"
            "Outline:\n{outline}"
        ),
        "max_tokens": 16384,
    },
    "peer_review": {
        "system": (
            "You are a rigorous reviewer for Q1 geotechnical engineering journals "
            "(Computers and Geotechnics, Géotechnique, IJNAMG, JGGE). "
            "You provide constructive but critical reviews that improve paper quality."
        ),
        "user": (
            "Simulate peer review from 3 expert perspectives for a geotechnical "
            "numerical simulation paper. Output markdown with:\n\n"
            "**Reviewer A** — Numerical Methods Expert\n"
            "**Reviewer B** — Geotechnical Domain Expert\n"
            "**Reviewer C** — Scientific Writing and Rigor Expert\n\n"
            "Each reviewer must provide: strengths, major concerns, minor concerns, "
            "and specific actionable revision requests.\n\n"
            "CHECK THESE CRITICAL ISSUES:\n"
            "1. TOPIC ALIGNMENT: Does the paper address {topic} throughout? "
            "Are all sections connected to the stated research objective?\n"
            "2. VALIDATION QUALITY: Is the numerical model validated against published "
            "experiments or benchmarks? Are quantitative agreement metrics reported "
            "(NRMSD, % error)? Is a single validation case sufficient?\n"
            "3. NUMERICAL MODEL COMPLETENESS: Is the mesh sensitivity study present? "
            "Are all boundary conditions, element types, and interface formulations "
            "fully described for reproducibility?\n"
            "4. CONSTITUTIVE MODEL: Are all parameters physically motivated and "
            "calibrated? Is the parameter table complete with units and sources?\n"
            "5. NOVELTY POSITIONING: Is the specific contribution relative to prior "
            "work clearly stated? Are appropriate prior studies cited and compared?\n"
            "6. CLAIM SUPPORT: Are all claims in the Abstract, Introduction, and "
            "Conclusions backed by specific numerical results in the paper?\n"
            "7. WRITING QUALITY: Is technical terminology used correctly? "
            "Are limitations stated explicitly? Is the discussion mechanistic "
            "(explaining WHY, not just WHAT)?\n"
            "8. CITATION PRACTICE: Are original papers cited (not just surveys)? "
            "Are recent publications (last 5 years) included?\n\n"
            "Paper draft:\n{draft}\n\n"
            "Supporting evidence:\n{experiment_evidence}"
        ),
        "max_tokens": 8192,
    ...
    "paper_revision": {
        "system": (
            "You are a geotechnical paper revision expert. Your task is to improve "
            "a manuscript for Q1 journal submission by addressing all reviewer comments "
            "while preserving the scientific content.\n\n"
            "REVISION PRIORITIES:\n"
            "1. STRENGTHEN VALIDATION: Add quantitative agreement metrics if missing; "
            "improve comparison figures; state NRMSD or % error explicitly.\n"
            "2. SHARPEN NOVELTY: Rewrite Introduction to more clearly distinguish "
            "this work from cited prior studies.\n"
            "3. COMPLETE THE NUMERICAL MODEL: Add missing mesh sensitivity details, "
            "boundary condition specifications, or constitutive parameter justifications.\n"
            "4. IMPROVE DISCUSSION: Make it mechanistic — explain physical reasons "
            "for observed behaviour, not just describe curves.\n"
            "5. FIX CLAIMS: Every claim in Abstract and Conclusions must be directly "
            "backed by specific numbers in the paper. Remove or qualify unsupported claims.\n"
            "6. CONSOLIDATE LIMITATIONS: Move all caveats to the Limitations section; "
            "remove scattered disclaimers from other sections.\n\n"
            "WRITING RULES:\n"
            "- Do NOT add new claims that are not supported by the existing results.\n"
            "- Do NOT fabricate numerical values or add results not in the draft.\n"
            "- Keep the Limitations section to 150-300 words listing 3-5 key points.\n"
            "- Use precise geotechnical terminology throughout.\n"
            "- CITATION FORMAT: All citations MUST remain in [cite_key] format.\n"
        ),
        "user": (
            "{academic_style_guide}\n"
            "{narrative_writing_rules}\n"
            "{anti_hedging_rules}\n"
            "{anti_repetition_rules}\n"
            "Revise the geotechnical paper draft to address all reviewer comments.\n"
            "Return the full revised paper in markdown only.\n\n"
            "CRITICAL REVISION REQUIREMENTS:\n"
            "- Address every major and minor concern raised by reviewers.\n"
            "- Preserve all sections and their core scientific content.\n"
            "- Improve quantitative validation metrics where flagged.\n"
            "- Strengthen the Introduction contribution statement.\n"
            "- Ensure the Conclusions contain specific quantitative findings.\n"
            "- Verify all citations are in [cite_key] format.\n"
            "- Target 6,000-9,000 words for the main body.\n"
            "{writing_structure}\n"
            "{topic_constraint}"
            "Draft:\n{draft}\n\nReviews:\n{reviews}"
        ),
    ...
    "quality_gate": {
        "system": (
            "You are a final quality gate evaluator for Q1 geotechnical engineering journals."
        ),
        "user": (
            "Evaluate this geotechnical journal paper for Q1 submission readiness. "
            "Return JSON with schema: {score_1_to_10:number, verdict:string, "
            "strengths:[...], weaknesses:[...], required_actions:[...]}.\n\n"
            "Evaluate specifically:\n"
            "- Validation quality (quantitative agreement with benchmarks)\n"
            "- Numerical model completeness and reproducibility\n"
            "- Constitutive model parameter justification\n"
            "- Novelty and contribution clarity\n"
            "- Scientific writing quality (no placeholders, no unsupported claims)\n"
